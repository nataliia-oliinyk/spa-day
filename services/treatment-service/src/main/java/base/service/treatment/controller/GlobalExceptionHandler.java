...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, List<String>>> handleImageFileException(ImageFileException ex) {
        List<String> errors = Collections.singletonList(ex.getMessage());
        return new ResponseEntity<>(getErrorsMap(errors), new HttpHeaders(), HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler({NotFoundException.class, NullPointerException.class})
    public ResponseEntity<Map<String, List<String>>> handleNotFoundException(Exception ex) {
        List<String> errors = Collections.singletonList(ex.getMessage());
